      ...
      <div className="min-h-screen bg-background">
        ...
        <div className="flex-1 p-8">
                ...
                {achievements.map((achievement, index) => (
                  <div key={index} className="space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs text-muted-foreground">{achievement.current}/{achievement.target}</span>
                    </div>
                    <Progress value={achievement.progress} className="h-2 bg-muted rounded-full">
                      <div 
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            {/* Urgent Community Needs - Enhanced */}
            <Card className="border-0 shadow-card bg-card hover:shadow-gentle transition-all duration-300 rounded-2xl flex flex-col">
              <CardHeader className="pb-4">
                <div className="flex items-center justify-between">
                  <CardTitle className="flex items-center gap-3 text-xl">
              ...
              <CardContent className="space-y-4 flex-1 flex flex-col">
                <div className="flex-1 space-y-4">
                  {communityNeeds.filter(need => need.urgency === "Immediate").map((need) => (
                    <div key={need.id} className="p-4 bg-transparent border border-border rounded-xl hover:shadow-gentle transition-all duration-200 group">
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex-1">
                          <h3 className="font-semibold text-foreground text-sm mb-1 line-clamp-1">{need.title}</h3>
                        ...
                        <Badge variant="destructive" className="rounded-full text-xs px-2 py-1 ml-2">
                          {need.urgency}
                        </Badge>
                      </div>
                      <div className="flex items-center justify-between text-xs text-muted-foreground mb-3">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <Button variant="ghost" size="sm" className="rounded-full h-6 px-3 text-xs group-hover:bg-primary group-hover:text-white" asChild>
                          <Link to="/browse">
                            Help <ChevronRight className="w-3 h-3 ml-1" />
                          </Link>
                        </Button>
                      </div>
                    </div>
                  ...
                  {recentActivity.map((activity) => {
                    ...
                    return (
                      <div key={activity.id} className="flex items-start gap-4 p-4 bg-transparent border border-border rounded-xl hover:shadow-gentle transition-all duration-200">
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center bg-${activity.color}-100 dark:bg-${activity.color}-900/20`}>
                          <IconComponent className={`w-4 h-4 text-${activity.color}-600 dark:text-${activity.color}-400`} />
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium text-foreground mb-1 line-clamp-1">{activity.title}</p>
