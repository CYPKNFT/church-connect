...
export function CollapsibleSidebar({ children }: CollapsibleSidebarProps) {
  ...
  // Only derive serving/giving mode from route; admin/adminCopy are click-controlled
  useEffect(() => {
    setIsServingMode(
          ...
          {!isAdminMode && !isAdminCopyMode && !isServingMode && !isGivingMode ? (
              ...
              <div className="px-4 space-y-1">
                {mainNavItems.map((item) => {
                  const isActive = currentPath === item.path || 
                    (item.isAdmin && currentPath.startsWith('/admin') && isAdminMode) ||
                    (item.isAdminCopy && isAdminCopyMode);
            ...
            <div className="flex">
              ...
              <div className="w-15 bg-sidebar border-r border-sidebar-border flex flex-col items-center py-4 space-y-2">
                ...
                {/* Navigation Icons */}
                {mainNavItems.map((item) => {
                  const isActive = (item.isAdmin && isAdminMode) || (item.isAdminCopy && isAdminCopyMode);
            ...
            /* STATE 3: ADMIN COPY EXPANDED - Icon strip + Admin copy submenu */
            <div className="flex">
              {/* Left Column - Icon Strip */}
              <div className="w-15 bg-sidebar border-r border-sidebar-border flex flex-col items-center py-4 space-y-2">
                {/* Brand Icon */}
              ...
              >
                ...
                {!isAdminCopyCollapsed && (
                  <div className="p-4 border-b border-sidebar-border">
                    <h2 className="font-semibold text-sidebar-foreground">Admin Copy</h2>
                    <p className="text-sm text-sidebar-foreground/70">Alternative Layout</p>
                  </div>
                )}
              ...
              >
                ...
                {!isGivingCollapsed && (
                  <div className="p-4 border-b border-sidebar-border">
                    ...
                    <p className="text-sm text-sidebar-foreground/70">Share & Discover</p>
                  </div>
                )}
